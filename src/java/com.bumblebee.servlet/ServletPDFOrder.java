...
public class ServletPDFOrder extends HttpServlet {
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) {
        ...
        try {
            ...
            if (orderCtrl.getOrderTotal().getOrderList().size() > 0) {

                Customer person = orderCtrl.getOrderTotal().getPerson();
                ...
                doc.add(new Paragraph(person.getFirstname() + " " + person.getLastname() + "\n" + person.getStreet() + " " + person.getHouseNo() + "\n" + person.getPostcode() + " " + person.getTown()));
                doc.add(Chunk.NEWLINE);
                doc.add(Chunk.NEWLINE);
                ...
                for (OrderedItem a : orderCtrl.getOrderTotal().getOrderList()) {

                    table.addCell("" + a.getFood().getFoodNo());
                    table.addCell(a.getFood().getName());
                    table.addCell(a.getFood().getIngredients());
                    table.addCell("" + a.getFood().getPrice() + " €");
                    table.addCell("" + a.getNumber());
                    table.addCell("" + a.getTotalLine() + " €");
                }

                doc.add(table);

                doc.add(new Paragraph("                   Gesamtbetrag: " + orderCtrl.getTotalAll() + " €", middle));
            ...
            Logger.getLogger(ServletPDFOrder.class.getName()).log(Level.SEVERE, null, ex);
